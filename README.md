...
| --- | --- |
| `Code` | Opens a project and launches a coding CLI in WSL; tool diagnostics run after agent selection so the selector opens quickly |
...
| `Light update` | Updates installed AI coding CLIs only, including Kilo Code CLI, DROID CLI, Grok CLI, and Command Code |
...
`Kilo Code CLI`, `Oh My Codex / OMX`, `Oh My OpenAgent`, `DROID CLI`, `Grok CLI`, `Command Code`, and `Oh My OpenCode Slim` remain separate optional installs from the install menu.
...
`Reset tool configs` opens a second selector. From there you can reset only `Codex / OMX`, `OpenCode`, `Oh My OpenAgent`, `Oh My OpenCode Slim`, `Claude Code`, `Gemini CLI`, `DROID CLI`, `Grok CLI`, `Command Code`, or `All tracked configs`.
